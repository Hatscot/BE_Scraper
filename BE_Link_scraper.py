...
def make_scraper():
    # Wir benutzen curl_cffi anstatt cloudscraper, da dies deutlich besser 
    # den Cloudflare Bot-Schutz umgeht, indem es einen echten Chrome-Browser 
    # TLS Fingerprint simuliert.
...
def scrape_theme(theme: dict, session) -> list[LegoSet]:
    ...
    while True:
        ...
        log.info("  → %d neue Sets geladen (gesamt: %d)", new_added, len(all_sets))

        # Wenn die Seite uns keine neuen Sets mehr geliefert hat (z.B. wegen veränderter Parametrisierung), brechen wir ab.
        if new_added == 0:
            log.info("  Keine neuwertigen Sets gefunden, wahrscheinlich Pagination-Ende.")
            break
